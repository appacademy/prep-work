...
def caesar_cipher(offset, string):
    ...
    while word_idx < len(words):
        ...
        while letter_idx < len(word):
            ...
            char_i = ord(word[letter_idx]) - ord("a")

            new_char_i = (char_i + offset) % 26
            word = list(word)
            word[letter_idx] = chr(ord("a") + new_char_i)
...
class CaesarCipher(unittest.TestCase):
    ...
    def test_simple_case(self):
        self.assertEqual(caesar_cipher(3, "abc"), "def")

    def test_with_space(self):
        self.assertEqual(caesar_cipher(3, "abc xyz"), "def abc")
